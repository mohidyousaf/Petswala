...
class MarketPlaceBloc extends Bloc<MarketPlaceEvent, MarketPlaceState> {
  ...
  MarketPlaceBloc() : super(MarketPlaceState.initial()) {
    on<ChangeSelectionEvent>((event, emit) {
      List<ProductItem> displayedProducts = state.products.where((element)=>
        element.category == event.category || event.category == 'All'
      ).toList();
      emit(MarketPlaceState.initial(products: state.products, displayedProducts: displayedProducts));
    });
    ...
    on<InitializeListEvent>((event, emit) async{
      ...
      emit(MarketPlaceState.initial(products: products, displayedProducts: products));
    });
  }
